...
- Сохраняйте сложные рисунки с несколькими слоями (оружия например) как файлы `*.kra` куда-нибудь в папку `images`. Это позволит потом легче их редактировать.
...
1. Создание темы. Движок требует все текстуры с тем же размером, с которым они будут рисоваться на базовом разрешении, так что этот принцип приходится нарушать в этой области. Рекомендуется иконки для тем рисовать в `.svg`.
